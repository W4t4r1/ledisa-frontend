// app/components/ConsultorChat.tsx
'use client'
...
export default function ConsultorChat({
  ...
  subtitle = 'Conexión Directa al Inventario',
  placeholder = 'Escribe tu consulta de stock o presupuesto...',
  welcomeMessage,
  className = 'flex flex-col h-[calc(100vh-6rem)] bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden',
    ...
    <div className={className}>
            ...
            {welcomeMessage || (
              <>
                ...
                <p className="text-sm mt-2">
                  ...
                  <em>"Un cliente necesita revestir un cuarto de 4x4 metros. Quiere usar el porcelanato Tambopata. Calcúlame exactamente cuántas cajas necesita y cuánto le va a costar."</em>
                </p>
              </>
            )}
          </div>
        ...
        {cargando && (
          <div className="flex justify-start">
            <div className="bg-white border border-gray-200 rounded-lg p-4 text-sm flex gap-2 items-center shadow-sm rounded-bl-none">
              <span className="font-bold text-[#04558C] animate-pulse">Analizando inventario...</span>
            </div>
          </div>
        )}
      ...
      <form onSubmit={enviarMensaje} className="p-4 bg-white border-t border-gray-200 flex gap-3">
        <input
          ...
          className="flex-1 border border-gray-300 rounded-md p-3 focus:outline-none focus:border-[#04558C] text-sm text-black"
          disabled={cargando}
        />
        <button
          type="submit"
          disabled={cargando || !input.trim()}
          className={`${buttonClassName} text-white px-6 py-3 rounded-md font-bold transition-colors disabled:opacity-50 shadow-sm text-sm`}
        >
